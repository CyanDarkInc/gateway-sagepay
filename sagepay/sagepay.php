 ...
 * Credit Cards.
 *
 * The Sage Pay API can be found at: http://integrations.sagepay.co.uk/content/getting-started-submit-payment-your-server
 *
 * @package blesta
...
class Sagepay extends MerchantGateway implements MerchantCc
{
    ...
    {
        return Language::_('Sagepay.name', true);
    }
    ...
    private function processTransaction($url, array $fields)
    {
        ...
        $this->Http->setHeader('Authorization: Basic ' . base64_encode($this->meta['integration_key'] . ':' . $this->meta['integration_password']));
    ...
    private function getCardIdentifier($url, $card_info, $merchant_session_key)
    {
        ...
        $params = $this->buildJson([
            'cardDetails' => [
                'cardholderName' => $this->ifSet($card_info['first_name']) . ' ' . $this->ifSet($card_info['last_name']),
                'cardNumber' => $this->ifSet($card_info['card_number']),
                'expiryDate' => substr($this->ifSet($card_info['card_exp']), 4, 2) . substr($this->ifSet($card_info['card_exp']), 2, 2),
        ...
        return $response['cardIdentifier'];
    }
